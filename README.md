...
and uploads it to a WordPress server.
...
* When you are given a `Client ID` and `Client secret` value, put these in a `.config.yaml` file. See `config.template.yaml` as
an example.
